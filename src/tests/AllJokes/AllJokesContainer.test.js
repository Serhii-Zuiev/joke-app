...
describe("<AllJokesContainer /> unit test", () => {
  ...
  it("should dispatch initial fetch jokes action", () => {
    getWrapper();
    expect(mockStore.dispatch).toHaveBeenCalledWith(fetchJokesRequested(20));
  });
  ...
  it("form should dispatch the correct action on submit", () => {
    wrapper.find(".form-control").simulate("change", { target: { value: 11 } });
    wrapper.find(".sc-Axmtr").simulate("submit");
    expect(mockStore.dispatch).toHaveBeenCalledWith(fetchJokesRequested(11));
  });
});
